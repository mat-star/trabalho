...
novoParagrafo.innerText = "Depois que você clicou, eu apareci !!"; document.body.appendChild(novoParagrafo);
}
// Remove o último parágrafo, se houver mais de um
function removerParagrafo() {
}
const paragrafos = document.querySelectorAll("p"); if (paragrafos.length > 1) {
paragrafos [paragrafos.length-1].remove();
} else {
}
alert("Não há mais parágrafos para remover 11");
// Muda a cor de fundo da página para uma cor aleatória function mudarCorFundo() {}

const corAleatoria = `#${Math.floor(Math.random() * 16777215).toString(16)}`; document.body.style.backgroundColor = corAleatoria;
